...
function updateUIForLanguage() {
    ...
    /* Toggles – use parent <label> instead of [for="..."] */
    const readLabel = document.getElementById("readAloudToggle")?.parentElement;
    const voiceLabel = document.getElementById("voiceModeToggle")?.parentElement;

    if (readLabel) {
        const span = readLabel.querySelector("span");
        if (span) span.textContent = ui.readAloud;
    }

    if (voiceLabel) {
        const span = voiceLabel.querySelector("span");
        if (span) span.textContent = ui.voiceMode;
    }
